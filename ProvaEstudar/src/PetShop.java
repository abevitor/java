...
public class PetShop {
    public static void main(String[] args) {
		...
		if(servico.toLowerCase().equals("banho")) {
            ...
            if(servico.toLowerCase().equals("credito")) {
                System.out.println("Quantas vezes voce deseja parcelar?");
                int parcela = scanner.nextInt();

                banhar(tamanho, shampoo);
                System.out.println("O metodo escolhido de pagamento foi " + pagamento + "e sera parcelado em: " + parcela + "vezes");

			
                return;
            }
            if(servico.toLowerCase().equals("pix")) {
			
                return;
            }
            if(servico.toLowerCase().equals("dinheiro")) {
			
                return;
            }
            if(servico.toLowerCase().equals("debito")) {
